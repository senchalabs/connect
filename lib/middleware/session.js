...
//<<<<<<< HEAD
    ...
    var generateIdentifier = function (fn) {
	  ...
	  console.log('store generated sid: ', sid); 
          fn(err, sid || null); 
        }); 
      } else { 
        var sid = utils.uid(24);
	console.log('self generated sid: ', sid); 
        fn(null, sid + "." + hash(sid, secret, fingerprint(req)));
      }  
    };
/*
=======
...
*/
    ...
    var generateNewSession = function (fn) {
      console.log('connect.generateNewSession');
    ...
    var loadExistingSession = function(sid, fn) {
      console.log('connect.loadExistingSession');
      store.get(sid, function (err, data) {
        if ((err && err.code == 'ENOENT') || !data) { 
	  console.log('no session found for', sid, err, data);
          generateNewSession(fn); 
          return;
        } else  if (err) { 
	   console.log('error loading session', err, data); 
	  return fn(err);
	} else { 
           // if the store successfully returned a session 
	  console.log('no session found', sid);
    ...
    if (!req.sessionID) {
      ...
      if (store.verify) { 
        ...
        // the store should definately provide a verify-func if store.generate 
        // does note generate ids in the schema [sessionid].[hashed-sessionid]
        store.verify( 
          req.sessionID, 
          secret, 
          fingerprint(req), 
        ...
        if (parts[1] == hash(parts[0], secret, fingerprint(req))) {
