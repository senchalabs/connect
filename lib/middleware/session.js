...
function session(options){
  ...
  var options = options || {}
    ...
    , fingerprint = options.fingerprint || function (req) {
      return req.headers['user-agent'] || req.client.remoteAddress+req.client.remotePort || '';
    }
    , cookie = options.cookie
    , hash = options.hash || function (req, base) {
      return crypto
  ...
  // sessionID generating function
  store.genSID = options.genSID || function (req, hashFn) {
      var base = utils.uid(24);
      var sessionID = base + '.' + hashFn(req, base);
      return sessionID;
  };

  // generates the new session
  store.generate = function(req){
    req.sessionID = store.genSID(req, store.hash);
