...
var send = exports.send = function(req, res, next, options){
  ...
  fs.stat(path, function(err, stat){
    // mime type
    type = mime.lookup(options.fileName||path);
    ...
    if(options.isAttachment||options.fileName){
      res.setHeader('Content-Disposition', (options.isAttachment ? 'attachment;': '' )+( options.fileName ? 'filename="'+encodeURIComponent(options.fileName)+'"':''));
    }
