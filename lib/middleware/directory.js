...
function iconStyle (files, useIcons) {
  ...
  var data = {};
  var extsUsed = {};
  var views = { tilesAndDetails: [], mobile: [] };
  ...
  for (var i=0; i < files.length; i++) {
    ...
    if (extsUsed[ext]) continue;
    extsUsed[ext] = 1;
    data[icon] = ext + ' .name{background-image: url(data:image/png;base64,' + load(icon)+');}';
    views.tilesAndDetails.push('.view-tiles ' + ext + ' .name, .view-details ' + data[icon]);
  	views.mobile.push('#files ' + data[icon]);
  }

  var style = views.tilesAndDetails.join('\n')
