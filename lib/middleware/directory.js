  ...
  return function directory(req, res, next) {
    ...
    fs.stat(path, function(err, stat){
      ...
      fs.readdir(path, function(err, files){
        ...
        {
          var file = join(path, files[f]);

          var stat = fs.statSync(file);
          if (stat.isFile())
          {
            fileResult.push({name:files[f], stat:stat});
          }
          else
          {
            folderResult.push({name:files[f], stat:stat});
          }
        }
        ...
        files = folderResult.sort(compareFiles).concat(fileResult.sort(compareFiles));
...
exports.html = function(req, res, files, next, dir, showUp, icons){
  fs.readFile(__dirname + '/../public/directory.html', 'utf8', function(err, str){
    ...
    fs.readFile(__dirname + '/../public/style.css', 'utf8', function(err, style){
      if (err) return next(err);
      if (showUp) files.unshift({name:'..', stat:null});
      str = str
        .replace('{style}', style)
...
exports.json = function(req, res, files){
  files = JSON.stringify(get_file_names(files));
...
exports.plain = function(req, res, files){
  files = get_file_names(files).join('\n') + '\n';
...
function html(files, dir, useIcons) {
  return '<ul id="files">' + files.map(function(file){
    ...
    if (useIcons)
    {
      if ('..' == file.name || file.stat && file.stat.isDirectory())
      {
        icon = 'folder-icon.png';
      }
      else
      {
        icon = icons[extname(file.name)] || icons.default;
      }
