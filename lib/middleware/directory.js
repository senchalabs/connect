...
function iconStyle (files, useIcons) {
  ...
  for (var i=0; i < files.length; i++) {
    ...
    if (!data[icon]) {
      ...
      styles[icon] = {
        selectors: '',
        rules: 'background-image: url(data:image/png;base64,' + data[icon] + ');'
      };
    }
    var comma = styles[icon].selectors.length > 0 ? ',' : '';
    styles[icon].selectors += comma + '#files .icon-' + ext.substring(1) + ' .name';
  }

  for (icon in styles) {
    style += styles[icon].selectors + '{' + styles[icon].rules + '}\n';
  }
