...
exports = module.exports = function directory(root, options){
  ...
  var hidden = options.hidden
    ...
    , sort = options.sort || {dirFirst: false}
...
exports.html = function(req, res, files, next, dir, showUp, icons, view){
  fs.readFile(__dirname + '/../public/directory.html', 'utf8', function(err, str){
    ...
    fs.readFile(__dirname + '/../public/style.css', 'utf8', function(err, style){
      if (err) return next(err);
      if (showUp) files.unshift({name: '..', size: '', mtime: ''});
      str = str
        .replace('{style}', style)
...
function iconStyle (files, useIcons) {
  ...
  for (var i=0; i < files.length; i++) {
    ...
    ext = file.isDirectory() ? '.directory' : (icons[ext] ? ext : '.default');
    var icon = icons[ext];
    ...
    for (var j=0; j < size.length; j++) {
      data[icon][size[j]] = ext
        + ' .name{background-image: url(data:image/png;base64,'
        + load(icon, size[j])+');}';
    }
    views.tiles.push('.view-tiles ' + data[icon]['16x16']);
    views.details.push('.view-details ' + data[icon]['16x16']);
...
function html(files, dir, useIcons, view) {
  return '<ul id="files" class="view-'+view+'">'
    ...
    var date = file.name == '..' ? ''
      : file.mtime.toDateString()+' '+file.mtime.toLocaleTimeString();
