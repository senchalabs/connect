...
Server.prototype.handle = function(req, res, out) {
  ...
  function next(err) {
    ...
    try {
      ...
      if (undefined == urlobj.pathname) urlobj.pathname = '/';

      // skip this layer if the route doesn't match.
      if (0 != urlobj.pathname.indexOf(layer.route)) return next(err);

      var nextChar = urlobj.pathname[layer.route.length];
      ...
      if ('/' != urlobj.pathname[0]) urlobj.pathname = '/' + urlobj.pathname;

      // Re-assembling url based on the modified "urlobj"
