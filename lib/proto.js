...
app.handle = function(req, res, out) {
  ...
  function next(err) {
    ...
    try {
      u = url.parse(req.url);
      if (undefined == u.pathname) u.pathname = '/';

      // skip this layer if the route doesn't match.
      if (0 != u.pathname.indexOf(layer.route)) return next(err);

      c = u.pathname[layer.route.length];
      if (c && '/' != c && '.' != c) return next(err);

      // Call the layer handler
      // Trim off the part of the url that matches the route
      removed = layer.route;
      u.pathname = u.pathname.substr(removed.length);

      // Ensure leading slash
      if ('/' != u.pathname[0]) u.pathname = '/' + u.pathname;

      // Re-assembling url based on the modified "u" object
