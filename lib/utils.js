...
/**
 * Sign and base64-encode the given `val` with `secret`.
 *
 * @param {String} val
 * @param {String} secret
 * @return {String}
...
exports.sign = function(val, secret){
  val = new Buffer(val).toString('base64').replace(/=+$/, '');
  return val + '.' + crypto
...
exports.unsign = function(val, secret){
  var parts = val.split('.')
    , str = new Buffer(parts[0], 'base64').toString()
    , sig = parts[1];
...
exports.parseJSONCookies = function(obj){
  Object.keys(obj).forEach(function(key){
    var val = obj[key];
    if (0 == val.indexOf('json')) {
      try {
        obj[key] = JSON.parse(val.slice(4));
      } catch (err) {
        // nothing
      }
    }
