...
exports.handle = function(err, req, res, next){
    ...
    if (!err && req.method === 'POST' && contentType === 'application/json') {
        ...
        req.addListener('end', function() {
            req.rawBody = data;
            try {
                var rpc = req.body = JSON.parse(data);
            } catch (err) {
                return respond(res, { error: { code: PARSE_ERROR }});
            }
            try {
                if (validRequest(req)) {
                    var method = me.procedures[rpc.method];
                    if (typeof method === 'function') {
                        var params = rpc.params || [];
                        params.push(function(err, result){
                            if (err) {
                                // TODO: handle predefined / custom errors
                            } else {
                                respond(res, {
                                    id: rpc.id,
                                    result: result
                        ...
                        method.apply(helpers, params);
                    } else {
                        respond(res, { id: rpc.id, error: { code: METHOD_NOT_FOUND }});
                    }
                } else {
                    respond(res, { id: rpc.id, error: { code: INVALID_REQUEST }});
                }
            } catch (err) {
                return respond(res, { id: rpc.id, error: { code: INTERNAL_ERROR }});
            }
        });
    } else {
        next();
...
function validRequest(req){
    return req.body.jsonrpc === VERSION
        && typeof req.body.id === 'number'
        && typeof req.body.method === 'string';
}

/**
 * Respond with the given JSON object.
 *
