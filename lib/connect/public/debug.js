...
document.addEventListener('DOMContentLoaded', function(){
    var debug = new Debug();
    debug.addPanel('Request', function(panel){
        panel.appendChild(createTable(req.headers));
    });
    
    debug.addPanel('Response', function(panel){
...
function Debug(){
    ...
    debug.innerHTML = '<div id="debug-header"><h2>Connect Debug</h2><ul id="debug-tabs"></ul></div>';
...
Debug.prototype.showPanel = function(panel){
    if (this.previousPanel) {
        this.element.removeChild(this.previousPanel);
    }
