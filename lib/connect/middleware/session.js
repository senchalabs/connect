...
exports = module.exports = function sessionSetup(options){
    ...
    return function sessionHandle(req, res, next) {
        ...
        res.writeHead = function(status, headers){
            ...
            // Only set/refresh session for particular URLs
            var updateSession=true;
            if (options.urlsMatch&&options.urlsMatch.length) {
                var uri=req.url.toString().replace(/\?.*$/,'');
                updateSession=false;
                for (var i in options.urlsMatch) {
                    if (uri.match(options.urlsMatch[i])) {
                        updateSession=true;
                        break;
                    }
                }
