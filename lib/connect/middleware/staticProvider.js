 ...
 *   - `exclude`  Regular expression that prevents sending for paths
 *                who pass the test method of the expression.
 *
...
module.exports = function staticProvider(options){
    ...
    return function staticProvider(req, res, next) {
        ...
        // Filter out any excluded content
        if (exclude) {
            if (exclude.test(url.pathname)) {
                console.log("restricting access to file:", url.pathname);
                return next();
            }
        }
