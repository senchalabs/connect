...
var filterDir = __dirname + "/filters"
fs.readdirSync(filterDir).forEach(function (filename) {
    if (/\.js$/.test(filename)) {
        var name = filename.substr(0, filename.lastIndexOf('.'));
        var path = filterDir + "/" + name;
        filters[name] = require(path);
    }
});
sys.debug(sys.inspect(filters));