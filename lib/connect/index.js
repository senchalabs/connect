...
Server.prototype.handle = function(req, res, outerNext) {
    ...
    res.writeHead = function (code, headers) {
        ...
        headers = headers || {};
        addHeader("Date", date.toUTCString());
        addHeader("X-Response-Time", (date - start) + "ms");
        if (me.showVersion) {
            addHeader("X-Powered-By", "Connect " + exports.version);
            addHeader("Server", "Node " + process.version);
        }
        res.writeHead(code, headers);

        function addHeader(key, value) {
          // Headers may be placed in dictionaries or arrays
